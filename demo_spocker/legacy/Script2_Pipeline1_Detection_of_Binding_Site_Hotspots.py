...
def find_pdb_file(pdb):
    candidates = [
        PDB_BASE / f"{pdb}.pdb",
        PDB_BASE / f"{pdb.lower()}.pdb",
        PDB_BASE / f"{pdb}.cif",
        PDB_BASE / f"{pdb.lower()}.cif",
    ]
    for c in candidates:
        if c.exists():
            return c
...
def find_iso_table(pdb_dir, pdb):
    preferred = pdb_dir / f"{pdb}_stk_hp_ele_selected_isovalues.csv"
    if preferred.exists():
        return preferred
    fallback = sorted(pdb_dir.glob("*_stk_hp_ele_selected_isovalues.csv"))
    if fallback:
        return fallback[0]
    return None
...
# -------------------------------------------------
# RNA residue / hydrogen helpers
# (fixes for pdbfixer-processed and raw RCSB PDBs)
# -------------------------------------------------

# Definitive DNA residue names – always excluded
_DNA_RESNAMES = {
    "DA", "DC", "DG", "DT", "DI",
    "ADE", "CYT", "GUA", "THY",
}
...
def _is_hydrogen_name(atomname: str) -> bool:
    """
    Return True for hydrogen (H) or deuterium (D) atom names.

    Handles:
      - Standard names    : H8, HO2', H5', H1
      - Digit-first names : 1H5', 2HN, 3H   (legacy PDB & pdbfixer convention)
      - Deuterium names   : D2', DO2', D8    (neutron diffraction structures)

    FIX vs. original: the original filter `startswith("H")` missed digit-first
    hydrogen names (added by pdbfixer) and all deuterium names.
    """
...
def _is_rna_residue(resname: str, atom_names: list) -> bool:
    """
    Return True only for RNA nucleotide residues.

    Priority order:
      1. Explicit O2' atom present  → definitive ribose (RNA) marker.
      2. Name in _DNA_RESNAMES      → reject.
      3. Name in _RNA_RESNAMES      → accept.
      4. Name starts with 'R' (≤4 chars) → accept (e.g. RA, RG in some PDBs).
      5. Otherwise                  → reject (protein, ion, unknown).

    FIX vs. original: no residue filter existed; protein/DNA atoms from
    mixed-chain PDBs (protein–RNA complexes) were included in atomxyz,
    corrupting buriedness and proximity scores.
    """
...
# -------------------------------------------------
# Structure loader  (fully corrected)
# -------------------------------------------------

def load_structure_atoms(pdb_path):
    """
    Return heavy-atom XYZ coordinates (Å) of RNA residues only,
    as a float64 array of shape (N, 3).

    Correct for:
      • Raw RCSB PDBs          – no explicit H atoms
      • pdbfixer-processed PDBs – all H atoms added (including digit-first
                                   names like 1H5', 2HN)
      • CIF files
      • Protein–RNA complexes  – protein residues excluded from atomxyz
      • Neutron diffraction    – deuterium (D) atoms excluded

    Changes vs. original load_structure_atoms():
      1. _is_hydrogen_name() replaces startswith("H"):
         catches digit-first (1H5', 2HN) and deuterium (D2').
      2. _is_rna_residue() filter added:
         only RNA heavy atoms enter atomxyz; protein and DNA excluded.
    """
    atoms = []
...
for pdb_dir in pdb_dirs:
    ...
    stk_file  = find_field_file(pdb_dir, pdb, "stacking")
    hyd_file  = find_field_file(pdb_dir, pdb, "hydrophobic")
    apbs_file = find_field_file(pdb_dir, pdb, "apbs")
    pdb_file  = find_pdb_file(pdb)
    iso_table = find_iso_table(pdb_dir, pdb)

    if stk_file is None or hyd_file is None or apbs_file is None:
        print(f"[SKIP] {pdb}: missing stacking, hydrophobic, or apbs .mrc in {pdb_dir}")
        continue
    if pdb_file is None:
        print(f"[SKIP] {pdb}: missing 3D structure in {PDB_BASE}")
        continue
    if iso_table is None:
        print(f"[SKIP] {pdb}: missing selected isovalues CSV in {pdb_dir}")
        continue
    ...
    save_mrc_from_mask(stk_mask,  stk_file,  pdb_dir / f"{pdb}.stacking_thresholded.mrc")
    save_mrc_from_mask(hyd_mask,  hyd_file,  pdb_dir / f"{pdb}.hydrophobic_thresholded.mrc")
    save_mrc_from_mask(apbs_mask, apbs_file, pdb_dir / f"{pdb}.apbs_thresholded.mrc")
    ...
    if best_mixed_mask is not None:
        save_mrc_from_mask(best_mixed_mask, stk_file,
                           pdb_dir / f"{pdb}.mixed_fields_pocket.mrc")
    ...
    if best_se_mask is not None:
        save_mrc_from_mask(best_se_mask, stk_file,
                           pdb_dir / f"{pdb}.stacking_electrostatic_pocket.mrc")
    ...
    if best_sh_mask is not None:
        save_mrc_from_mask(best_sh_mask, stk_file,
                           pdb_dir / f"{pdb}.stacking_hydrophobic_pocket.mrc")
    ...
    if second_sh_mask is not None:
        save_mrc_from_mask(second_sh_mask, stk_file,
                           pdb_dir / f"{pdb}.stacking_hydrophobic_second_pocket.mrc")
    ...
    if best_ele_mask is not None:
        save_mrc_from_mask(best_ele_mask, stk_file,
                           pdb_dir / f"{pdb}.electrostatic_pocket.mrc")
    ...
    per_pdb_df.to_csv(
        pdb_dir / f"{pdb}.predicted_binding_pockets_summary.csv", index=False)

if summary_rows:
    summary_df = pd.DataFrame(summary_rows).sort_values(["pdb", "pocket_type"])
    summary_df.to_csv(
        BASE / "predicted_binding_pockets_summary_all_pdbs.csv", index=False)
    print("Saved:", BASE / "predicted_binding_pockets_summary_all_pdbs.csv")
    print("Saved per PDB folder:")
    for f in [
        ".stacking_thresholded.mrc",
        ".hydrophobic_thresholded.mrc",
        ".apbs_thresholded.mrc",
        ".mixed_fields_pocket.mrc",
        ".stacking_electrostatic_pocket.mrc",
        ".stacking_hydrophobic_pocket.mrc",
        ".stacking_hydrophobic_second_pocket.mrc",
        ".electrostatic_pocket.mrc",
        ".predicted_binding_pockets_summary.csv",
    ]:
        print(f"  - {f}")
else:
    print("No PDB folders were processed successfully.")
