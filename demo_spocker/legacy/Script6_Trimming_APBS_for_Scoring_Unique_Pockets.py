...
and keeps the APBS map only around all RNA heavy atoms using:
...
def find_apbs_mrc(analysis_dir: Path, pdb_id: str):
    primary = analysis_dir / f"{pdb_id}.apbs.mrc"
    if primary.exists():
        return primary
...
def process_pdb(pdb_id: str, cutoff_a: float, verbose: bool = True) -> bool:
    ...
    apbs_path = find_apbs_mrc(pdb_analysis_dir, pdb_id)
    if apbs_path is None:
        if verbose:
            print(f"  [SKIP] No APBS MRC found in {pdb_analysis_dir}")
        return False

    pdb_file = find_pdb_file(pdb_id)
    if pdb_file is None:
        if verbose:
            print(f"  [SKIP] No PDB/CIF file found for {pdb_id} in {PDB_DIR}")
        return False
    ...
    trimmed_path = pdb_analysis_dir / f"{pdb_id}.apbs_rna_trimmed.mrc"
...
def main():
    ...
    for pdb_id in pdb_ids:
        print(f"\n[{pdb_id}]")
        try:
            ok = process_pdb(pdb_id, cutoff_a=args.cutoff, verbose=True)
            if ok:
                success += 1
            else:
                skipped += 1
