...
def discover_pdb_names(base_dir):
    ...
    A sub-directory qualifies as a PDB folder when it contains at least one
    of the three expected MRC input files.  Hidden directories are ignored.
    """
    names = []
    for entry in sorted(os.listdir(base_dir)):
        ...
        if not os.path.isdir(full_path) or entry.startswith("."):
            continue
        has_mrc = any(
            os.path.isfile(os.path.join(full_path, f"{entry}.{suffix}"))
            for suffix in ("apbs.mrc", "hbacceptors.mrc", "hbdonors.mrc")
        )
        if has_mrc:
            names.append(entry)
    return names
...
def resample_to_master(src_data, src_vs, src_origin,
    ...
    xa_s, ya_s, za_s = make_axes(src_data, src_vs, src_origin)   # FIX: za_s not xa_s
...
# ─────────────────────────────────────────────────────────────────────────────
# CONNECTIVITY STRUCTURE BUILDER
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# POCKET DETECTION: TRIPLE OVERLAP + SPATIALLY SEPARATED CLUSTERING
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# SITE PATCH: SINGLE CONNECTED TRIPLE-OVERLAP COMPONENT
# ─────────────────────────────────────────────────────────────────────────────
...
def process_structure(name, base_dir):
    """
    Run the full pipeline for one PDB.

    Parameters
    ----------
    name     : str  — PDB identifier (folder name under base_dir)
    base_dir : str  — root path; both inputs and outputs live in
                      <base_dir>/<name>/
    """
    print(f"\n{'='*60}")
    ...
    # ── Resolve per-PDB directory (input == output) ───────────────────────────
    pdb_dir  = os.path.join(base_dir, name)
    ele_file = os.path.join(pdb_dir, f"{name}.apbs.mrc")
    hba_file = os.path.join(pdb_dir, f"{name}.hbacceptors.mrc")
    hbd_file = os.path.join(pdb_dir, f"{name}.hbdonors.mrc")
    ...
    # Output goes into the same per-PDB directory
    out_subdir = pdb_dir
    print(f"  Input / Output directory: {out_subdir}")
...
if __name__ == "__main__":
    print("hbond_pocket_finder.py  v6")
    print(f"  BASE_DIR (input + output): {BASE_DIR}")
    print(f"\nActive parameters:")
    ...
    if not names:
        ...
        print("    <name>.apbs.mrc  |  <name>.hbacceptors.mrc  |  <name>.hbdonors.mrc")
        raise SystemExit(1)

    print(f"\nDiscovered {len(names)} PDB folder(s): {', '.join(names)}")

    for name in names:
        process_structure(name, BASE_DIR)
