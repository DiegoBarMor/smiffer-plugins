...
# ─────────────────────────────────────────────────────────────────────────────
# HYDROGEN FILTER  ← THE KEY FIX
# ─────────────────────────────────────────────────────────────────────────────

def is_hydrogen_name(atomname: str) -> bool:
    """
    Return True if atomname corresponds to a hydrogen or deuterium atom.

    PDB/mmCIF convention:
      • Hydrogens: name starts with "H" (after stripping whitespace and
        normalising * → ')
      • Deuteriums: name starts with "D"
      • Some legacy PDBs use "1H…", "2H…", "3H…" (the digit-first variant).
        These are caught by checking whether any character in the name is
        "H" or "D" after removing leading digits — but a simpler safe rule
        is: if the name is entirely [0-9HD'] characters it is a hydrogen.

    We use the conservative rule: strip the name, uppercase it, and check
    whether the first non-digit character is H or D.
    """
    name = atomname.strip().upper().replace("*", "'")
    # Remove leading digits (e.g. "1H5'", "2HN")
...
# ─────────────────────────────────────────────────────────────────────────────
# MASKS / COMPONENTS / CENTRE  (unchanged from V3)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# STRUCTURE LOADING  — hydrogen atoms are now excluded in all paths  ← FIX
# ─────────────────────────────────────────────────────────────────────────────

def find_structure_file(pdb_id):
    candidates = [
        PDB_BASE / f"{pdb_id}.pdb",
        PDB_BASE / f"{pdb_id.lower()}.pdb",
        PDB_BASE / f"{pdb_id}.cif",
        PDB_BASE / f"{pdb_id.lower()}.cif",
    ]
    for c in candidates:
        if c.exists():
            return c
...
def grouped_atoms_to_xyz(grouped_atoms):
    """
    Collect XYZ coordinates for all heavy atoms (non-H, non-D) of RNA residues.
    Hydrogen atoms are excluded here so that trim_sphere_by_atoms uses only
    heavy-atom positions regardless of how the PDB was prepared.        ← FIX
    """
    atoms = []
    for (_, _, _, resname), atom_list in grouped_atoms.items():
        ...
        for atomname, x, y, z in atom_list:
            if is_hydrogen_name(atomname):          # ← FIX: skip H/D atoms
                continue
            atoms.append((x, y, z))
    if not atoms:
...
def load_structure_atoms_xyz_manual_pdb(structure_path):
    ...
    return grouped_atoms_to_xyz(grouped)   # H/D filtered inside grouped_atoms_to_xyz
    ...
    for res in u.residues:
        ...
        for atom in res.atoms:
            ...
            if altloc not in ("", "A", None):
                continue
            if is_hydrogen_name(atom.name):         # ← FIX: skip H/D atoms
                continue
...
# ─────────────────────────────────────────────────────────────────────────────
# VOLUME ALGORITHM  (unchanged from V3)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# POCKET PROCESSING  (unchanged from V3, except summary now reports H count)
# ─────────────────────────────────────────────────────────────────────────────
...
def process_single_pocket(pdb_id, pocket_path, pocket_tag, pocket_label,
    ...
    sphere_mask, n_excluded = trim_sphere_by_atoms(
        ...
        atom_xyz=atom_xyz,          # atom_xyz already contains only heavy atoms
        voxel_size_zyx=voxel_size_zyx,
        origin_xyz=origin_xyz,
        excl_radius_a=ATOM_EXCLUSION_RADIUS_A
    )
...
for pdb_dir in pdb_dirs:
    pdb_id         = pdb_dir.name
    structure_file = find_structure_file(pdb_id)

    if structure_file is None:
        print(f"Skipping {pdb_id}: missing RNA structure file in {PDB_BASE}")
        continue

    atom_xyz = load_structure_atoms_xyz(structure_file)
    if len(atom_xyz) == 0:
        print(f"Skipping {pdb_id}: no RNA heavy atoms parsed from structure file "
              f"{structure_file}")
        continue
    ...
    for spec in POCKET_SPECS:
        pocket_path = pdb_dir / f"{pdb_id}{spec['suffix']}"
    ...
    if not existing_pockets:
        print(f"Skipping {pdb_id}: no eligible pocket MRC files found in {pdb_dir}")
        continue
    ...
    summary_txt = pdb_dir / f"{pdb_id}.all_processed_pocket_volumes_summary.txt"
    ...
    all_summary_paths.append(summary_txt)
    print("Summary TXT:", summary_txt)

print("\nFinished processing all available PDB folders.")
for p in all_summary_paths:
    print("Generated summary:", p)
