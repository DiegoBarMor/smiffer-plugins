...
=================================================================
For each PDB folder under ANALYSIS_BASE:
  ...
  • Saves each accepted trimmed mask as:
        <pdb_dir>/unique_pockets/<pdb>.Pocket1_Volume.mrc
        <pdb_dir>/unique_pockets/<pdb>.Pocket2_Volume.mrc  … etc.
  ...
  • A pocket is considered a TERMINAL POCKET (and therefore discarded)
    if MORE THAN terminal_voxel_fraction (default 0.50) of its voxels
    lie within TERMINAL_ZONE_CUTOFF_A (default 8.0 Å) of any heavy atom
  ...
  ──────────────────────────────
  • Reads field files (from pdb_dir) used for weight calculation and scoring:
  ...
  OUTPUTS (per PDB, inside unique_pockets/)
  ──────────────────────────────────────────
...
# -------------------------------------------------
# GLOBAL FONT: Times New Roman throughout all plots
# -------------------------------------------------
...
# HBOND HELPERS
# -------------------------------------------------
def find_hbond_dir(pdb):
    for name in [pdb, pdb.upper(), pdb.lower()]:
        d = HBOND_BASE / name
        if d.is_dir():
            return d
    if HBOND_BASE.is_dir():
        pdb_lower = pdb.lower()
        for entry in HBOND_BASE.iterdir():
            if entry.is_dir() and entry.name.lower() == pdb_lower:
                return entry
    return None
...
def collect_hbond_sites(pdb, hbond_dir):
    sites = []
    folder_stem = hbond_dir.name
    stems = []
    for s in [folder_stem, pdb, pdb.upper(), pdb.lower()]:
        if s not in stems:
            stems.append(s)
...
def load_all_label_masks(pdb, pdb_dir, hbond_dir):
    all_found_labels = []
    label_masks = {}
    label_voxels = {}
    label_origins = {}
...
# -------------------------------------------------
# COMPLETE-LINKAGE / MAXIMAL CLIQUE GROUPING
# -------------------------------------------------
...
# -------------------------------------------------
def load_terminal_residue_atoms(pdb_dir, pdb):
    """
    ...
    for name in [pdb, pdb.upper(), pdb.lower()]:
        for ext in [".pdb", ".cif"]:
            for search_dir in [pdb_dir, PDB_DIR]:
                fp = search_dir / f"{name}{ext}"
                if not fp.exists():
                    continue
                try:
                    ...
                    if len(atoms) > 0:
                        print(f"  [Term]  Loaded {len(atoms)} terminal heavy-atom coords "
                              f"from {fp.name}")
                        return atoms
                    # File parsed successfully but yielded 0 terminal atoms —
                    # fall through to try other names/extensions rather than
                    # returning an empty array prematurely.
                except Exception as e:
                    print(f"  [WARN]  Could not parse terminal residues from "
                          f"{fp.name}: {e}")
    print(f"  [WARN]  No structure file found for terminal-residue detection "
          f"({pdb}) — terminal filtering will be skipped.")
...
def find_field_file(pdb_dir, pdb, primary_suffix, fallback_suffix):
    for name in [pdb, pdb.upper(), pdb.lower()]:
        f = pdb_dir / f"{name}{primary_suffix}"
        if f.exists():
            return f, "trimmed"
    if fallback_suffix is not None:
        for name in [pdb, pdb.upper(), pdb.lower()]:
            f = pdb_dir / f"{name}{fallback_suffix}"
            if f.exists():
                return f, "raw_fallback"
    return None, None


def load_raw_field_data(pdb_dir, pdb):
    fields = {}
    for primary_suffix, fallback_suffix, fname in RAW_FIELD_SPECS:
        fp, variant = find_field_file(pdb_dir, pdb, primary_suffix, fallback_suffix)
        if fp is None:
...
def load_rna_atoms(pdb_dir, pdb):
    for name in [pdb, pdb.upper(), pdb.lower()]:
        for ext in [".pdb", ".cif"]:
            for search_dir in [pdb_dir, PDB_DIR]:
                fp = search_dir / f"{name}{ext}"
    ...
    print(f"  [WARN]  No PDB/CIF structure file found for {pdb} — distance trimming will be skipped.")
    return np.zeros((0, 3), dtype=float)
...
# -------------------------------------------------
def main():
    pdb_dirs = sorted(p for p in ANALYSIS_BASE.iterdir() if p.is_dir())
    print(f"Processing {len(pdb_dirs)} PDB folder(s) under {ANALYSIS_BASE}\n")

    for pdb_dir in pdb_dirs:
        pdb = pdb_dir.name
        print(f"=== {pdb} ===")

        hbond_dir = find_hbond_dir(pdb)
        if hbond_dir is None:
            print("  [HBond] No HBond folder found — skipping HBond sites")

        atom_xyz = load_rna_atoms(pdb_dir, pdb)

        all_found_labels, label_masks, label_voxels, label_origins = \
            load_all_label_masks(pdb, pdb_dir, hbond_dir)
        ...
        terminal_atom_xyz = load_terminal_residue_atoms(pdb_dir, pdb)
        ...
        if discarded_pockets:
            ...
            print(f"  [Term]  No terminal pockets detected — all pockets retained.")
        ...
        raw_field_data = load_raw_field_data(pdb_dir, pdb)
        ...
        unique_dir = pdb_dir / "unique_pockets"
        ...
        for pocket in unique_pockets:
            if pocket["merged_mask"] is None or pocket["voxel"] is None:
                ...
                continue
            out_path = unique_dir / f"{pdb}.{pocket['filename']}"
            try:
                write_mrc(out_path,
        ...
        contrib_csv = unique_dir / f"{pdb}_field_contributions.csv"
        try:
        ...
        if unique_pockets and any(pfi for pfi in pocket_field_integrals):
            plot_path = unique_dir / f"{pdb}_field_contributions.png"
            try:
