"""
  Inputs:
    <PDB_DIR>/<name>.pdb                              — RNA structure
    <BASE_DIR>/<name>/<name>.HBond_Site_N_marker.mrc  — marker MRCs

  Outputs (written to the per-PDB MRC sub-folder):
    <BASE_DIR>/<name>/<name>.HBond_Site_N_pocket_volume.mrc
    <BASE_DIR>/<name>/<name>.pocket_volume_summary.txt
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# AUTO-DISCOVER ALL PDB SUB-FOLDERS
# ─────────────────────────────────────────────────────────────────────────────

def discover_pdb_names(base_dir):
    """
    Return a sorted list of PDB names found under base_dir.
    A sub-directory qualifies when it contains at least one marker MRC file.
    ...
    for entry in sorted(os.listdir(base_dir)):
        ...
        has_marker = any(
            os.path.isfile(
                os.path.join(full_path, f"{entry}.HBond_Site_{n}_marker.mrc")
            )
...
# ─────────────────────────────────────────────────────────────────────────────
# PDB PARSER  —  v2: robust column-slice reader
# ─────────────────────────────────────────────────────────────────────────────
...
def build_seed_sphere(centre_a, vs, origin, grid_shape,
    ...
    sphere_mask[iz_lo:iz_hi+1, iy_lo:iy_hi+1, ix_lo:ix_hi+1] =         dist2_a <= radius_a**2
...
# ─────────────────────────────────────────────────────────────────────────────
# STEP 4  —  TRIM SPHERE BY RNA ATOMS
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# STEPS 5-6  —  LABEL FRAGMENTS AND SELECT POCKET PIECE
# ─────────────────────────────────────────────────────────────────────────────
...
def process_structure(name, base_dir, pdb_dir):
    """
    Run the full pocket-volume pipeline for one PDB.

    Parameters
    ----------
    name     : str  — PDB identifier (sub-folder name under base_dir)
    base_dir : str  — root path for MRC sub-folders and outputs
                      (<base_dir>/<name>/ holds marker MRCs and receives outputs)
    pdb_dir  : str  — flat directory where <name>.pdb files live
    """
    print(f"\n{'='*60}")
    ...
    # ── Resolve paths ──────────────────────────────────────────────────────────
    mrc_subdir = os.path.join(base_dir, name)          # MRC inputs + outputs
    pdb_path   = os.path.join(pdb_dir,  f"{name}.pdb") # PDB read from PDB_DIR

    if not os.path.isfile(pdb_path):
        print(f"  [SKIP] PDB not found: {pdb_path}")
        return

    if not os.path.isdir(mrc_subdir):
        print(f"  [SKIP] MRC sub-directory not found: {mrc_subdir}")
        return
    ...
    for site_n in range(1, MAX_SITES + 1):
        ...
        mrc_path = os.path.join(mrc_subdir, mrc_name)
    ...
    if not marker_files:
        print(f"  [SKIP] No marker MRC files found in {mrc_subdir}")
        return
    ...
    for site_n, marker_path in marker_files:
        ...
        out_mrc_path = os.path.join(mrc_subdir, out_mrc_name)  # output → MRC folder
    ...
    # Write summary into the per-PDB MRC sub-directory
    summary_path = os.path.join(mrc_subdir, f"{name}.pocket_volume_summary.txt")
...
if __name__ == "__main__":
    print("hbond_pocket_volume.py  v2")
    print(f"  BASE_DIR (MRC sub-folders + outputs) : {BASE_DIR}")
    print(f"  PDB_DIR  (flat PDB files)             : {PDB_DIR}")
    print(f"\nActive parameters:")
    ...
    # Auto-discover all PDB sub-folders under BASE_DIR
    names = discover_pdb_names(BASE_DIR)
    if not names:
        print(f"\n[ERROR] No valid PDB sub-folders found in: {BASE_DIR}")
        print("  Each sub-folder must contain at least one of:")
        print("    <name>.HBond_Site_1_marker.mrc  |  <name>.HBond_Site_2_marker.mrc")
        raise SystemExit(1)

    print(f"\nDiscovered {len(names)} PDB folder(s): {', '.join(names)}")

    for name in names:
        process_structure(name, BASE_DIR, PDB_DIR)
